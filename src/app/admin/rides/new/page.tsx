...
export default function NewRidePage() {
  ...
  return (
    <div className="animate-fade-in">
      <h2 className="text-lg md:text-2xl font-bold text-admin-text mb-4 md:mb-6">Nova Corrida</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
            ...
            <div>
              ...
              <input id="origin" type="text" value={form.origin} onChange={(e) => handleChange("origin", e.target.value)} placeholder="Ex: Aeroporto FLN" className="admin-input w-full" />
            </div>

            <div>
              <label htmlFor="destination" className="text-[10px] text-admin-muted uppercase tracking-widest mb-1.5 block">Destino</label>
              <input id="destination" type="text" value={form.destination} onChange={(e) => handleChange("destination", e.target.value)} placeholder="Ex: Hotel Majestic" className="admin-input w-full" />
            </div>
          ...
          <div className="flex gap-2 pt-2">
            <button type="button" onClick={() => router.back()} className="flex-1 sm:flex-none bg-admin-card border border-admin-border text-admin-text-dim hover:text-admin-text hover:bg-admin-card-hover rounded-lg px-4 py-2.5 text-sm transition">
              Cancelar
            </button>
            <button type="submit" className="flex-1 sm:flex-none btn-admin py-2.5">
              Criar Corrida
            </button>
          </div>
        </form>
