...
export function NewRideModal({ open, onClose, onCreated }: NewRideModalProps) {
  ...
  const handleCreate = useCallback(async () => {
    ...
    if (name) {
      ...
      if (existing) {
        ...
        const { data: newClient, error: clientErr } = await supabase
          .from("clients")
          .insert({ name, agency_id: form.agency_id || null })
          .select("id")
          .single();
        if (clientErr) {
    ...
    if (form.date && form.time) {
      ...
      scheduled_at = `${form.date}T00:00:00`;
    } else {
    ...
    if (error) {
      toast.error("Erro ao criar corrida: " + error.message);
      return;
    }
  ...
  return (
    ...
    >
      <div
        className="bg-admin-dark border border-admin-border rounded-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto p-5 md:p-6 animate-fade-in"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-admin-text">Nova Corrida</h3>
          <button onClick={onClose} className="text-admin-muted hover:text-admin-text p-1">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="space-y-3">
          ...
          <button onClick={handleCreate} className="btn-admin w-full py-2.5 mt-1">
            Criar Corrida
          </button>
        </div>
      </div>
